...
void ProgressBars::updateFoodLeft()
{
	...
	float progressZeroOne = (totalFood - (float)foodLeft) / (float)totalFood;

	float progressTimer = UTIL::lerp(0, -(originalBarHeight), progressZeroOne);
	float progressTimerScale = UTIL::lerp(1, 0, progressZeroOne);
	float progressTimerTop = UTIL::lerp((originalBarHeight), -(originalBarHeight), progressZeroOne);

	float resolutionPos = (visibleSize.height/2);
	float newPosition = resolutionPos + progressTimer;

	foodProgressMiddle->setPosition(Vec2(sideOffset, newPosition));
	foodProgressMiddle->setScale(1, progressTimerScale);

	foodProgressTop->setPosition(Vec2(sideOffset, (visibleSize.height/2) + progressTimerTop + topOffset));
}
